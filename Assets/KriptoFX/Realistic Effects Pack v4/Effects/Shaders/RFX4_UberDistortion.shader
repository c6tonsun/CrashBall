	...
	[PerRendererData]	_Cutout("Cutout", Range(0, 1.2)) = 1
		...
		SubShader
		{
			...
			Pass
			{
				...
				half2 offset = dist.rg * UNITY_ACCESS_INSTANCED_PROP(_Distortion) * _GrabTexture_TexelSize.xy * i.color.a * fade;
				...
				cutoutCol.a = 1;
	#ifdef USE_CUTOUT
				half cutout = UNITY_ACCESS_INSTANCED_PROP(_Cutout);
				...
				half4 result;
				half4 mainCol = UNITY_ACCESS_INSTANCED_PROP(_MainColor);
				result.rgb = grabColor * lerp(1, mainCol,  i.color.a) + fresnelCol * grabColor + cutoutCol.rgb;

	#ifdef _EMISSION
				half4 tintCol = UNITY_ACCESS_INSTANCED_PROP(_TintColor);
